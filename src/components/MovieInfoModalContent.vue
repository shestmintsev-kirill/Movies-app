...
export default {
  ...
  props: {
    movie: {
      type: Object,
      required: true
    } //эту Пропсу я запрашиваю у MoviesList
  },
  data: () => ({
    defaultPosterBg:
  ...
  computed: {
    posterStyle() {
      return {
        "background-image": this.posterBg //возвращаю обьект в котором буду использовать "background-image", которую буду подставлять на сам элемент ч-з атрибут :style
      };
    },
    posterBg() {
      return this.movie ? `url(${this.movie.Poster})` : this.defaultPosterBg; //формирую текущий BG для компоненты
    }
  },
  methods: {
    closeModal() {
      this.$emit("closeModal"); //эмичу событие закрития мадального окна
    }
  }
};
</script>
